...
export const TableFunctions = ({
      ...
      const columnSearch = (columnName, filterValue) => {
        ...
        if (column) {
          ...
          if (item[columnName].id) {
            return item[columnName][column.selectableField].toLowerCase() === filterValue.toLowerCase();
  ...
  const getUniqueValues = (columnName, data, engineField) => {
    ...
    data.forEach((item) => {
      if (item[columnName] !== undefined) {
        if (item[columnName].id) {
          uniqueValues.add(item[columnName][engineField].toString().toLowerCase());
        } else {
  ...
  const uniqueValues = (columnName, engineField) => {
    ...
    uniqueValuesByColumn[columnName] = getUniqueValues(columnName, data, engineField);
