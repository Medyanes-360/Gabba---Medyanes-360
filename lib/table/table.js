...
hide: true ise tabloda görünmez.z
...
export const companyColumn = [
  ...
  },
  {
    ...
    cell: (data) => {
      return data.status ? (
        <span class='inline-flex gap-2 items-center text-md font-medium px-3 py-0.5 rounded-full bg-green-600 text-white'>
          <span class='w-2 h-2 mx-1 bg-green-100 rounded-full'></span>
          Aktif Şirket
        </span>
      ) : (
        <button
          onClick={async () => {
            await postAPI('/companyUpdate', data, 'PUT');
            data.toast.success('Şirket aktif edildi ve mağazalar güncellendi!');
            await data.getData();
          }}
          class='inline-flex gap-2 items-center text-md font-medium px-3 py-0.5 rounded-full bg-red-600 text-white transition-all duration-300 hover:scale-105 hover:transition-all hover:duration-300'
        >
          <span class='w-2 h-2 mx-1 bg-red-100 rounded-full'></span>
          {data.status ? 'Aktif Şirket' : 'Şirketi Aktif Et'}
        </button>
      );
    },
  },
